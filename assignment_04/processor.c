...
void fetch(int id) {
    proc_id=id;
    if(PC[id]<0 || PC[id]+3>=INSTRUCTION_MEMORY_SIZE) { end_of_simulation[id]=1; return; }
    opcode=Instruction[id][PC[id]];
    dest=Instruction[id][PC[id]+1];
    src1=Instruction[id][PC[id]+2];
    src2=Instruction[id][PC[id]+3];
    PC[id]+=4;
}
void decode(void) {}

static void log_print(int id, int reg) {
    if(fd_log) fprintf(fd_log, "Process id: %d : x%d : %08X\n", process_pid[id], reg, (uint32_t)Register[id][reg]);
    fflush(fd_log);
}
