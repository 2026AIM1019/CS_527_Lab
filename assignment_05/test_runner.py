...
def lab4_multiprocess():
    ...
    originals = []
    try:
        # Lab 5 physical memory has 15 allocatable frames. Use one 512-byte
        # data page per demo process: 1 instruction frame + 1 data frame.
        # Six tasks therefore fit four processors + two waiting tasks.
        for i in range(1, 7):
        ...
        loaded = all(f"Loaded PID {i} on processor {i-1}" in out
                     for i in range(1, 5))
