...
void os_init(void)
{
    ...
    /* Lab 5: initialize the shared physical-memory/MMU state once. */
    memory_system_init();
...
int loader(const char *program_file, const char *data_file)
{
    ...
    if (p >= 0) {
        /*
         * Lab 5 adds a second resource: physical frames. A processor can
         * only receive the task after its instruction/data pages have been
         * mapped successfully. If physical memory is temporarily full,
         * leave the task in WAITING so it can be retried after a task
         * finishes and releases its frames.
         */
    ...
    for (int p = 0; p < NP; ++p) {
        ...
        for (int i = 0; i < MAX_PROCESSES; ++i) {
            ...
            if (initialize(p, tasks[i].program_file, tasks[i].data_file) != 0)
                continue; /* still waiting for enough physical frames */
